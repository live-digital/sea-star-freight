...
class FreightOperation(models.Model):
    ...
    operation_line_ids = fields.One2many(
        "freight.operation.line", "operation_id", string="Order", copy=False
    )
    ...
    act_inv_payment = fields.Float(
        ...
        help="Actual Receivable Payment",
    )
    ...
    def _check_date(self):
        for operation in self:
            ...
            if operation.exp_send_date and operation.exp_send_date < order_date:
                ...
                raise UserError(
                    _(
                        "Actual Send date should be greater or "
                        "equals to Order Date !!"
                    )
                )
    ...
    def _check_loading_discharging_port(self):
        for operation in self:
            if (
                ...
                raise UserError(
                    _("Loading Port and Discharging Port Should be Different !!")
                )
        ...
        for operation in self:
            for line in operation.operation_line_ids:
                ...
                if order_weight > line.container_id.weight:
                    raise UserError(
                        _(
                            "[ %s ] Container's Weight Capacity is"
                            " %s !\nYou planned more weight then "
                            "container capacity !!"
                        )
    ...
    def create(self, vals):
        ...
        if not vals.get("operation_line_ids", False):
            raise UserError(
                _(
                    "You can not create or duplicate "
                    "Direct Shipping without Order Lines !!"
                )
            )
        return rec
...
class FreightOperationLine(models.Model):
    ...
    sale_price = fields.Float(
        compute="_compute_calculate_sale_price", string="Sale Price", store=True
    )
    ...
    operation_id = fields.Many2one("freight.operation", string="Operation", copy=False)
    ...
    def _check_date_received_date(self):
        ...
        for route_l in self:
            if (
                ...
                raise UserError(
                    _(
                        "Routes 'Received Date' Must be less or"
                        " equals to 'Send Date' !!"
                    )
                )
    ...
    def _check_source_destination_location(self):
        ...
        for route_l in self:
            if (
                ...
                raise UserError(
                    _("Routes 'From' and 'To' Location" " Should be Different !!")
                )
...
class OperationService(models.Model):
    ...
    sale_total = fields.Float(
        compute="_compute_sale_total", string="Total Sale", store=True
    )
    cost_total = fields.Float(
        compute="_compute_cost_total", string="Total Cost", store=True
    )
    ...
    def _check_qty_and_price(self):
        ...
        for service in self:
            ...
            if service.list_price < 0:
                raise UserError(
                    _("You can't enter Sale Price in Negative" " for service!!")
                )
            if service.cost_price < 0:
                raise UserError(
                    _("You can't enter Cost Price in Negative" " for service!!")
                )
